...
::: {.columns}
::: {.column width="55%"}
**Per-stage ablation (5 variants × 30 pages):**

| Best variant per model | Mean CER |
|------------------------|---------:|
| Sonnet — grayscale_soft ¹ | 0.272 |
| Tesseract — grayscale_soft ¹ | 0.369 |
| Gemini Flash — raw / deskew_only | 0.564 / 0.544 |

¹ deskew + denoise (NL-means) + contrast (CLAHE), no binarize.

**Two patterns:**

- **Binarize is universally destructive** — every binarize cell is worse than that model's raw cell. Tesseract hurt MOST (+21pp).
- **Other stages are model-dependent.** Sonnet & Tesseract benefit from grayscale-soft; Gemini Flash does not.

**Why binarize fails:** collapses 256 grayscale levels → 2 with 0% mid-tones, stripping the gradient OCR needs.
:::

::: {.column width="45%"}
![](figures/results/preprocessing_ablation.png){width="100%"}
:::
:::
...
**A 30-year-old classical OCR beats Google's flagship vision LLM by 18 pp on Telugu.**
