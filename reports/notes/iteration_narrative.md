...
- **~~Per-stage preprocessing ablation.~~ DONE (Phase 5, last day).** We implemented the two deferred stages (denoise + contrast) and ran 9 new OCR cells covering deskew-only, grayscale-soft (deskew+denoise+contrast), and all-4-stages variants. The ablation is the headline of Section 6.3 of the final report. The remaining sub-experiment that we did NOT run is **alternative binarization** — Otsu's global threshold or a less aggressive adaptive `block_size`/`c` to test whether the lesson is "no binarization" or "softer binarization." Given how strongly the data points at "no binarization," we suspect softer settings would still hurt, but this remains empirically untested.
