...
Per the project specification's academic-integrity policy, the team used the following LLM-based assistants during development. The team reviewed and understands every line of submitted code and can explain any portion on request.

- **Claude Code (Anthropic)** — software-engineering assistant. Used interactively for design discussion, code generation, code review, and running analysis scripts, and via autonomous PR-producing dispatches in isolated git worktrees with multi-agent review (code-reviewer + refactoring-evaluator + standards-auditor + quality-control). The team reviewed and merged every PR; no commit carries `Co-Authored-By` AI attribution per `docs/standards/git_workflow_standard.md`.
- **ChatGPT (OpenAI), Perplexity, Google Search and Google Scholar** — background research, citation discovery, and reference lookups during planning. No code or report prose was authored verbatim by these tools.

In addition, the **Gemini 2.5 Flash, Claude Sonnet 4.6, and Claude Opus 4.8 APIs** are used AS the OCR pipeline being evaluated — they are the *subject* of the empirical comparison documented in Sections 5 and 6, not engineering assistants.
