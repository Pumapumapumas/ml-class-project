...
- **Claude Code (Anthropic)** — skills, rules, agents, and workflows for code generation, code review, and autonomous engineer-dispatch Pull Requests. Larger-scope work went through multi-agent review (code-reviewer + refactoring-evaluator + standards-auditor + quality-control) before each PR was opened; the team reviewed and merged every PR. No commit carries `Co-Authored-By` AI attribution.
