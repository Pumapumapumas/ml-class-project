...
We built an end-to-end Telugu OCR pipeline that compares four OCR systems (Tesseract 5 as the classical baseline, Google Gemini 2.5 Flash, Anthropic Claude Sonnet 4.6, and Anthropic Claude Opus 4.8) across up to five image-preprocessing variants on a 30-page evaluation subset stratified by image quality. We also developed an LLM-based validation framework that estimates OCR quality without ground truth, using fluency scoring and cross-model agreement.

Three empirical findings came out of a 510-row CER/WER matrix (4 OCR systems by 5 preprocessing variants on the eval subset, with Opus only on raw and preprocessed for cost reasons):

1. **Binarization is universally harmful, but other preprocessing is model-dependent.** A per-stage ablation across five variants (raw, deskew-only, deskew+denoise+contrast which we call "grayscale-soft", deskew+binarize, and all 4 stages) shows that any cell including the binarize stage performs worse than that model's raw cell. Tesseract suffered the most, at +21 percentage points CER versus raw. The other three stages (deskew, CLAHE-based contrast enhancement, and non-local-means denoising) all preserve grayscale information, and they help some models while hurting others. Claude Sonnet and Tesseract both reach their best CER under the grayscale-soft variant. Gemini Flash is best under raw, and is actually hurt by both contrast and denoising. The right preprocessing is per-model-class, not universal.
2. **Claude Sonnet 4.6 is the cost-quality sweet spot.** Opus 4.8 is only about 1 percentage point better on mean CER than Sonnet, but costs 7 times more per call. Gemini Flash 2.5 is roughly 28 percentage points worse than either Claude model.
3. **The classical Tesseract baseline beats Gemini Flash by 18 percentage points** on Telugu. Vision LLMs are not automatically superior to classical methods for low-resource scripts.

We also produced a 524+ page submission sample (over the spec's 500-page minimum) using Gemini 2.5 Flash. Total API spend across the project came out to approximately \$10.

The report is organized around the *process* of producing these findings, not the findings alone, following the instructor's guidance in Announcement 3 that documenting iteration matters more than the final result.
...
Our original plan was to define quality buckets a priori (faded text, skew, etc.) and then sample. After browsing a stratified pre-sample of 97 pages, we changed the methodology: hold off on bucket definitions until we had inspected the acutal data. Two findings emerged that we would have missed otherwise.
...
Per the project specification's academic-integrity policy, the team used the following LLM-based tools during development.
...
- **ChatGPT (OpenAI)**, general troubleshooting and assistance with external apps, complex commands, and managing environment setups across multiple operating systems.
