...
Sonnet 4.6, and Anthropic Claude Opus 4.8 --- across two
image-preprocessing conditions on a 30-page evaluation subset stratified
...
\item
  ...
  contract.
\item
  A 240-row CER/WER matrix
  (\texttt{data/processed/eval\_subset/cer\_wer.csv}) computed over four
  models × two preprocessing conditions × 30 stratified pages.
\item
  A 415-page submission sample of full-corpus Telugu OCR via Gemini
  Flash.
\item
...
Recent multimodal large language models --- Google Gemini (Google 2026),
Anthropic Claude (Anthropic 2026), OpenAI GPT-4o, and Qwen-VL --- are
...
For this project we work with a \textbf{5-book, 415-page subset} rather
than the full corpus, to keep iteration cycles fast within the four-day
delivery window. The subset choice is disclosed and discussed in Section
9 (Limitations).
...
\item
  \textbf{Binarize} --- OpenCV \texttt{cv2.adaptiveThreshold} with
  \texttt{block\_size=11,\ c=2} (mean Gaussian neighborhood, subtractive
  constant), Gaussian local adaptive thresholding. Output is
  single-channel uint8 with two values: 0 (text) and 255 (background).
\end{enumerate}
...
We deferred two stages originally planned (denoise + contrast
enhancement) for time. \textbf{The empirical matrix later showed this
deferral was net positive} --- see Section 6.
...
\item
  ...
  the \texttt{google-generativeai} SDK.
\item
  \texttt{src.ocr.claude.ClaudeAdapter} --- Anthropic Claude Sonnet 4.6
  by default, with constructor override for Opus 4.8.
\end{itemize}
...
matrix and the 415-page submission sample without re-paying API costs
for pages already done.
...
The full eval matrix is 4 models × 2 preprocessing conditions × 30
stratified pages = 240 rows. Aggregate statistics per cell:
...
Gemini Flash 2.5 (mean) & \textbf{0.564} & 0.544¹ & 0.750 & 3.45² &
0.725 \\
...
¹ Gemini deskew\_only mean is essentially tied with raw; median 0.450 is
within noise of raw's 0.444. ² Gemini all\_4\_stages mean is
outlier-driven (3 pages with CER \textgreater{} 2.0 --- hallucination
bursts; median 0.682 is the representative number).
...
and \textasciitilde17,000 pages; our 5-book subset of 415 pages is
roughly 2.4\% of the full corpus. Extrapolating per-page cost and
...
the overall fluency signal. The intuition matches: a page where
independent OCR systems converge on similar text is probably an easy
page (low CER); a page where they wildly disagree is probably hard (high
CER). Pearson r = -0.206 confirms the same direction; the larger
Spearman-vs-Pearson gap indicates the relationship is non-linear, which
is expected --- agreement saturates near 1.0 for easy pages and drops
off rapidly on hard ones.
...
\textbf{At-scale fluency validation on the full 415-page corpus.} Beyond
the 240-row eval matrix, we also ran the LLM fluency judge on every page
of the Gemini submission sample. The resulting distribution of 414
ratings (one page failed to score and was excluded) is shown below:
...
415-page unlabeled corpus --- the no-ground-truth quality estimator
...
Beyond the evaluation subset, we ran Gemini 2.5 Flash on the full
415-page 5-book corpus, producing per-page \texttt{.txt} outputs at
\texttt{data/processed/submission/gemini/}. 413 of 415 pages succeeded
on the first pass; 2 transient Google 500 errors retried successfully on
a second idempotent pass. Total wall-clock time: under 2 hours.
...
\textbf{5-book, 415-page subset} to keep iteration cycles fast within
the four-day window. Section 9 (Limitations) discusses the trade-off
honestly.
...
The final matrix is 4 models × 2 preprocessing × 30 pages = 240 rows.
Section 6 reports the per-cell findings.
...
\tightlist
\item
  \textbf{Five-book subset, not the full corpus.} A book in the full
  221-book corpus might present visual conditions absent from our five
  books. The quality taxonomy might miss buckets.
\item
  \textbf{Submission sample is 415 pages, below the project spec's
  stated minimum of 500.} This is a direct consequence of the 5-book
  scope choice --- the five books happen to total 415 paired image-text
  pages. We chose 5 books for iteration speed within the four-day window
  and accepted the resulting shortfall against the spec's 500-page
  minimum as a deliberate trade-off. A reproducer with more time could
  add a 6th book from the upstream HuggingFace corpus to clear the
  500-page bar without changing any code (see
  \texttt{scripts/download\_dataset.py}).
\item
...
This project was developed with substantial assistance from Claude Code
(Anthropic's CLI agent for software-engineering tasks), used in two
modes:
...
\tightlist
\item
  \textbf{Interactive PM-with-engineer mode.} Eric operated as the
  project PM; Claude operated as the senior engineering partner ---
  proposing designs, implementing code, reviewing diffs, and running
  scoring jobs. All decisions, code reviews, and merges went through
  Eric explicitly.
\item
  \textbf{Autonomous engineer dispatch.} For larger scope items (the
  preprocessing pipeline, the OCR adapter interface, the Gemini adapter,
  the logging-config retrofit, the Claude adapter), Eric dispatched
  headless Claude Code instances in isolated git worktrees with task
  specifications; each produced a Pull Request that Eric reviewed and
  merged. Multi-agent review (code-reviewer + refactoring-evaluator +
  standards-auditor + quality-control) ran before each PR was opened.
\end{enumerate}

No commits in this repository carry ``Co-Authored-By: Claude''
attribution; Eric is the listed author of every commit. This is a
deliberate disclosure choice --- AI tooling use is documented here, not
in \texttt{git\ blame} --- per
\texttt{docs/standards/git\_workflow\_standard.md}.
...
\item
  ...
  \texttt{f2bd895b4932b648174a8f47066f4166469933a0}. Other class teams
  converged on the same dataset (see
...
Anthropic. 2026. \emph{{Claude Sonnet 4.6} Model Documentation}.
...
Google. 2026. \emph{{Gemini 2.5 Flash} Model Documentation}.
...
\textbf{Wall-clock totals.} Eval matrix (240 OCR calls): approximately
40 minutes of API time across the four cells, dominated by Claude Opus
(\textasciitilde60 minutes for its 60 calls if run serially; ran in
parallel against the matrix-fire harness). The 415-page Gemini Flash
submission run took approximately 2 hours wall-clock against the paid
tier. Tesseract on 60 eval pages took approximately 4 minutes total
(per-page Docker startup overhead dominates the actual recognition
time).
